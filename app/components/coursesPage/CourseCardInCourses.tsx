...
export default function CourseCardInCourses({ course }: CourseProps) {
  return (
    <div className="w-full min-w-93.25 border border-gray-100 bg-white rounded-lg">
      <div className="flex flex-col gap-3 p-5 h-full">
        <div className="flex min-h-45.25 min-w-83.25 rounded-lg relative overflow-hidden">
          <Image
            src={course?.image || ""}
            alt={course?.title}
            fill
        ...
        <div className="flex flex-col gap-1 h-full">
          ...
          <p className="mt-1 min-h-17 font-semibold text-gray-900 text-[24px]">
          ...
          <div className="flex justify-between items-center text-gray-400 font-normal mt-auto">
            <div className="flex flex-col text-xs">
              Starting from
              <span className="font-semibold text-gray-700 text-[24px]">
                ${Math.floor(Number(course?.basePrice))}
              </span>
            </div>
            <Link
              href={`/courses/${course.id}`}
              className="px-4 py-3 font-medium hover:opacity-80 bg-[#4F46E5] text-white rounded-lg"
            >
              Details
            </Link>
          </div>
