    ...
    <div className="flex flex-col pr-22.5 gap-8">
      <div className="flex items-center justify-between">
        ...
        <div
          ...
          className="flex items-center cursor-pointer text-gray-400 hover:text-gray-500 gap-1"
        >
      ...
      <div className="w-full flex flex-col gap-14 text-gray-500">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex gap-1 flex-wrap">
            {loading ? 
            [...Array(5)].map((_, idx) => <FilteringItemsSkeleton key={idx}/>) 
            : 
            categories.map((category) => (
              <div
                ...
                className={`flex gap-1 h-10 rounded-lg p-1 px-2 cursor-pointer
      ${
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex gap-1 flex-wrap">
            {loadingTopics ? 
            [...Array(17)].map((_, idx) => <FilteringItemsSkeleton key={idx}/>) 
            : topics.map((topic) => (
              <div
                key={topic.id}
                onClick={() =>
                  selectingToggle(topic.id, selectedTopics, setSelectedTopics)
                }
                className={`flex gap-1 h-10 rounded-lg p-1 px-2 cursor-pointer
                ${
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex gap-2 flex-col items-start">
            {loading ? 
            [...Array(7)].map((_, idx) => <FilteringItemsSkeleton key={idx}/>) 
            : 
